...
final class UInputPadDevice {
    ...
    private var bindingMap: PadBindingMap
    private let tapHoldMicroseconds: useconds_t = 25_000

    init(bindingMap: PadBindingMap = .default) throws {
    ...
    func emit(_ action: PadAction) throws {
        ...
        case .panEnded:
            guard isPanning else { return }
            isPanning = false
            for code in binding.keyCodes.reversed() {
                try key(code, 0)
            }
            try sync()
        }
    ...
    func updateBindingMap(_ nextMap: PadBindingMap) throws {
        try release()
        bindingMap = nextMap
    }
    ...
    func release() throws {
        isPanning = false
        let codes = Set(bindingMap.bindings.values.flatMap(\.keyCodes))
        for code in codes {
            try key(code, 0)
        }
        try sync()
