...
struct ControlOptionsPanel: View {
    ...
    @State private var showsComputerManager = false
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            ControlOptionsRow(
                title: "Computer",
                subtitle: selectedComputerSubtitle,
                systemName: "desktopcomputer"
            ) {
                Button("Manage") {
                    showsComputerManager = true
                }
            }
        ...
        .padding(16)
        .sheet(isPresented: $showsComputerManager) {
            ComputerManagerSheet(connection: connection)
        }
    }

    private var selectedComputerSubtitle: String {
        guard let selectedServerID = connection.selectedServerID else {
            return "Not selected"
        }
        return connection.trustedServerName(id: selectedServerID) ?? "Selected"
    }
}

struct ComputerManagerSheet: View {
    @ObservedObject var connection: TabletConnection
    ...
    var body: some View {
        NavigationStack {
            List {
                Section("Authorized Computers") {
                    if connection.trustedServers.isEmpty {
                        Text("No computers authorized yet.")
                            .foregroundStyle(.secondary)
                    } else {
                        ForEach(connection.trustedServers) { server in
                            VStack(alignment: .leading, spacing: 8) {
                                HStack {
                                    Label(server.name, systemImage: selectedIcon(for: server))
                                    Spacer()
                                    if connection.selectedServerID == server.peerID {
                                        Text("Selected")
                                            .font(.caption)
                                            .foregroundStyle(.secondary)
                                    }
                                ...
                                HStack {
                                    Button("Connect") {
                                        connection.reconnectToTrustedServer(id: server.peerID)
                                        dismiss()
                                    }
                                    .buttonStyle(.borderedProminent)

                                    Button("Forget", role: .destructive) {
                                        connection.forgetTrustedServer(id: server.peerID)
                                    }
                ...
                Section("Nearby Computers") {
                    if connection.discoveredServers.isEmpty {
                        Text("No InkWand Server found on this Wi-Fi network.")
                            .foregroundStyle(.secondary)
                    } else {
                        ForEach(connection.discoveredServers) { server in
                            HStack {
                                VStack(alignment: .leading, spacing: 3) {
                                    Text(server.name)
                                    Text(connection.isTrusted(server) ? "Already authorized" : "Tap to request access")
                                        .font(.caption)
                                        .foregroundStyle(.secondary)
                                }
                                Spacer()
                                Button(connection.isTrusted(server) ? "Connect" : "Request") {
                ...
                if !connection.trustedServers.isEmpty {
                    Section {
                        Button("Forget All Computers", role: .destructive) {
                            connection.forgetAllTrustedServers()
                        }
                    }
                }
            }
    ...
    private func selectedIcon(for server: TrustedPeer) -> String {
        connection.selectedServerID == server.peerID ? "checkmark.circle.fill" : "desktopcomputer"
    }
}
